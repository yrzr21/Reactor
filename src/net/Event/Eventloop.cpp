...
int createTimerfd(int heartCycle) {
    ...
    memset(
        &timeout, 0,
        sizeof(
            struct itimerspec));  // 必须清0，不然timerfd无效----鬼知道什么原因
...
Eventloop::Eventloop(bool isMainloop, int maxGap, int heartCycle)
    : ep_(new Epoll),
      ...
      tfd_(createTimerfd(heartCycle)),
      tChannel_(new Channel(this, tfd_)),
      isMainloop_(isMainloop),
      maxTimeGap_(maxGap),
      heartCycle_(heartCycle),
      stop_(false)

{
    ...
    this->tChannel_->setreadcallback(
        std::bind(&Eventloop::handleTimeout, this));
}
...
void Eventloop::run() {
    ...
    while (!this->stop_)  // 事件循环。
    {
        std::vector<Channel *> rChannels =
            this->ep_->loop(10 * 1000);  // 等待监视的fd有事件发生。
        if (rChannels.size() == 0)
            this->epollTimeout_cb_(this);
        else {
            for (auto ch : rChannels) ch->handleEvent();
        }
    }
}
...
void Eventloop::updateChannel(Channel *ch) { this->ep_->updatechannel(ch); }

void Eventloop::removeChannel(Channel *ch) { this->ep_->removeChannel(ch); }

// 判断当前线程是否是I/O线程，即原本的通信事件循环
bool Eventloop::isInLoop() { return syscall(SYS_gettid) == this->loop_tid; }
...
void Eventloop::handleTimeout() {
    if (!this->isMainloop_) {
        ...
        {  // lock
            ...
            for (auto iter = this->connections_.begin();
                 iter != this->connections_.end();) {
    ...
    timeout.it_value.tv_sec =
        this->heartCycle_;  // 定时时间，固定为5，方便测试。
    timeout.it_value.tv_nsec = 0;
